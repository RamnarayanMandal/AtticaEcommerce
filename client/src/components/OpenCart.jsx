...
const OpenCart = () => {
  ...
  return (
    <div className="flex">
      ...
      <div className={`fixed right-0 top-0 h-full w-2/5 bg-white z-50 transform transition-transform ease-in-out duration-300 ${openDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Drawer header */}
        <div className="items-center px-10 py-8">
                ...
                <div key={index} className='flex items-center gap-20'>
                  ...
                  <div className='flex flex-col gap-4'>
                    ...
                    <div className='flex items-center justify-between text-2xl '>
                      <button className='bg-gray-300 w-14 h-10 rounded-lg' onClick={handleDecrement(item.itemNo)}>-</button>
                      <p className='m-5'>{item.quantity}</p>
                      <button className='bg-gray-300 w-14 h-10 rounded-lg text-xl items-center ' onClick={handleIncrement(item.itemNo)}>+</button>
                    </div>
          ...
          {/* Total Price */}
          <div className='sticky bottom-10   w-auto flex items-center justify-between bg-blue-500 py-4 px-2 rounded-xl '>
            <p className='text-2xl font-bold text'>{cartMessage ? cartMessage : `Total Price: ${subTotal}`}</p>
            <button onClick={handleSubmit} className='btn-bg hover:text-white font-bold py-2 px-4 rounded-xl relative'>Place Order</button>
          </div>
        </div>
      </div>
    </div>
